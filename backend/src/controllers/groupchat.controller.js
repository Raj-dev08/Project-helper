...
export const sendMessages = async (req, res,next) => {
    try {
        ...
        const group = await Projects.findById(groupId);

        if(!group) {
            return res.status(404).json({ message: "Group not found" });
        }

        if(!group.contributors.some(contributor => contributor.contributor.toString() === user._id.toString()) && group.creator.toString()!==user._id.toString()) {
